...
export default async function HowItWorksPage() {
  ...
  return (
    <main>
      <PageHero eyebrow="Fonctionnement" title="Vous savez quoi transmettre, quoi attendre et qui contacter" description="Airproche vous aide a trouver un chauffeur. Celui-ci confirme directement le prix, la disponibilite et le trajet." />
      ...
      <section className="border-y border-slate-200 bg-slate-50 py-16 sm:py-24"><div className="site-container grid gap-6 md:grid-cols-3">{[{ title: "Avant de vous engager", text: "Le chauffeur recapitulera le trajet, son prix et ses conditions." }, { title: "Paiement direct", text: "Le paiement est organise directement avec le chauffeur ; Airproche ne l encaisse pas." }, { title: "Avant le trajet", text: "Les consignes utiles et, lorsqu’ils sont disponibles, les détails du chauffeur sont communiqués." }].map((item) => <article key={item.title} className="surface-card p-7"><h2 className="font-extrabold text-slate-950">{item.title}</h2><p className="mt-3 text-sm leading-6 text-slate-600">{item.text}</p></article>)}</div></section>
