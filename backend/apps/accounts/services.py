...
from apps.notifications.services import send_account_notification
...
def send_verification_email(user: User, raw_token: str) -> bool:
    return send_account_notification(user, raw_token, purpose=AccountToken.Purpose.VERIFY_EMAIL)


def send_password_reset_email(user: User, raw_token: str) -> bool:
    return send_account_notification(user, raw_token, purpose=AccountToken.Purpose.RESET_PASSWORD)
