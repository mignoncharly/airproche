...
export default async function DriverPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main><PageHero eyebrow="Chauffeur independant" title={driver.display_name} description={driver.bio || "Profil professionnel verifie par la plateforme."} />
    <section className="site-container grid gap-8 py-16 lg:grid-cols-[0.8fr_1.2fr]"><div className="surface-card p-6"><h2 className="text-xl font-black">Couverture publiee</h2><p className="mt-4 text-sm text-slate-600">Aeroports : {driver.airports.map((a) => a.name).join(", ") || "sur demande"}</p><p className="mt-2 text-sm text-slate-600">Zones : {driver.service_areas.map((a) => a.name).join(", ") || "sur demande"}</p><p className="mt-5 text-sm leading-6 text-slate-600">Airproche ne collecte pas le paiement du trajet. Aucun envoi de formulaire ne vaut reservation.</p></div>{driver.accepts_quote_requests ? <InquiryForm driverId={driver.public_id} airports={driver.airports} /> : null}</section></main>;
}
